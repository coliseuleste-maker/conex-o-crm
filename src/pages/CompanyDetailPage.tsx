...
import {
  ArrowLeft, Plus, Users, DollarSign, TrendingUp, Target, Percent, Building2,
} from "lucide-react";
...
export default function CompanyDetailPage() {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: "", origin: "", responsible: "", potentialValue: "", notes: "",
  });
  ...
  const totalInNegotiation = companyLeads
  ...
  const statusData = kanbanStages.map((s) => ({
    name: s.label,
    value: companyLeads.filter((l) => l.status === s.id).length,
  }));

  const originData = companyLeads.reduce<Record<string, number>>((acc, l) => {
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {company.responsible && ` · Responsável: ${company.responsible}`}
            </p>
          </div>
          <Badge variant={company.status === "active" ? "default" : "secondary"} className="ml-2">
                ...
                {companyLeads.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                      Nenhum lead cadastrado para esta empresa.
                    </TableCell>
                  </TableRow>
                )}
                {companyLeads.map((l) => (
          ...
          <div className="flex gap-3 overflow-x-auto pb-4">
            {kanbanStages.map((stage) => {
              ...
              return (
                <div
                  key={stage.id}
                  className="flex-shrink-0 w-56 bg-muted/50 rounded-lg p-3"
        ...
        <TabsContent value="relatorios">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {originChartData.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-12">Sem dados de origem</p>
                ) : (
                  <ResponsiveContainer width="100%" height={280}>
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold">Comissões — Leads Fechados</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {closedLeads.length === 0 && (
                        <TableRow>
                          <TableCell colSpan={4} className="text-center text-muted-foreground py-6">Nenhum lead fechado ainda.</TableCell>
                        </TableRow>
                      )}
                      {closedLeads.map((l) => (
                        <TableRow key={l.id}>
                          ...
                          <TableCell className="text-sm">{company.commissionRate}%</TableCell>
                          <TableCell className="text-sm font-semibold text-success">
                            {formatCurrency((l.potentialValue * company.commissionRate) / 100)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
